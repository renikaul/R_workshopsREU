...
ggplot(passengers1, aes(x = Pclass, fill = Surv_yn)) +
  geom_bar() +
  scale_fill_manual(values = c("#3399FF", "#333333"))
  
  Pclassprop <- passengers %>%
  group_by(Pclass) %>%
  summarise(propServ = sum(Survived) / n(),
  StandardError = sd(Survived) / sqrt(n()))
  
  ggplot(Pclassprop, aes(x = Pclass, y = propServ)) +
  geom_col() + scale_fill_manual(values = c ("#3399FF")) +
  geom_errorbar(aes(
  ymin = propServ - StandardError,
  ymax = propServ + StandardError
  ))
  
  #run anova test comparing survival rate by Pclass
  pclass.aov <- aov(Survived ~ as.factor(Pclass), data = passengers1)
  summary(pclass.aov)
  
  #since the statistical value is less than .05, it can be concluded that at least one pair of survival rates is statistically diffrent.
  
  #run Tukey-Kramer test to determine which means are diffrent
  TukeyHSD(pclass.aov)
  
  #after running the Tukey-Kramer test, it can be concluded that all the values are statistically diffrent from one another.
```
