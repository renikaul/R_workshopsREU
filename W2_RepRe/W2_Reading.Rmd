...
**Note the hash is interpereted differently inside (comment) and outside (header) of code chunks**
...
The `.Rmd` can also compile HTML code for more extensive formatting needs (ie. text color).
...
```{r, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE, tidy=TRUE, fig.k}
library(tidyverse)
...
> The mean age of passanges that survived  is 'r round(survived_Age, digits=1)`.

-OR-

> The mean age of passanges that survived  is 32.9.
