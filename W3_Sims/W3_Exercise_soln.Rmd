...
The `ggplot()` function requires data in a data.frame. What is the current object class? Based on the reading for today, how can we convert `sim1` to a data.frame?
...
- Revisiting reproducible reseach and workflow 
...
An alternative is creating a script and storing it in the `scripts/` folder of your working directory. Scripts have a `.R` file extension, and are a way to reference more stable code without actually interacting with the stable code. Let's make a script for the `ExponentialGrowth()` function. 
...
Is the `ExponentialGrowth()` function in your environment?


**Recap:**

- You've simulated and plotted data for exponential growth and SIR model
- Explored the impact of parameter values on the SIR model
- Wrote a script and sourced to current environment 
