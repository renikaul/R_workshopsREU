...
passengers %>% summary()  #this is another way to summarize the data
...
#this removes na from the data set, not just from the summary
...
#NOW to actually have this manipulated data set
males <- passengers %>%
  filter(Sex == "male")
...
Men are less likely to survive! I mean, think titanic
NOW at this point we have not done ANYTHING with the data (it is unmanipulated) because we have not saved a manipulated version
...
passengers %>%
  arrange(desc(Fare))
#this is actually nesting functions!
```
...
passengers %>% 
  mutate(Surv_yn = ifelse(Survived == 0, "No", "Yes"))

#just make a new column so you don't mess up
...
passengers1 <- passengers %>% #saving to a new data frame!
  mutate(Surv_yn = ifelse(Survived == 0, "No", "Yes")) #yes vs no
...
ggplot(passengers1, aes(x = Sex)) + #aes = aesthetics
  geom_bar()
...
ggplot(passengers1, aes(x = Age, y = Fare)) + geom_point()

#ggplot uses the + instead of the pipe!!!! (%>%)
#which is apparently the guys biggest regret
...
#we're going to make 2 plots! wooooooah
...
ggplot(passengers1, aes(x = Sex, fill = Surv_yn)) +
  geom_bar()
...
passengers %>% 
  group_by(Sex) %>% 
  summarise(meanFare = mean(Fare), propSurv = sum(Survived)/n())
...
2.  Talk through how the data needs to be manipulated to get the appropriate answer. Would plotting the data be useful? Which verbs will you use and in what order? 
 - plotting the data would be great --> we will make a side by side bargraph
 - x axis = 3 classes
 - y axis = proportion who survived
 - verbs = Groupby, facet_grid, geom_bar, filter
...
passengers %>% 
  group_by(Pclass) %>% 
  summarise(Propsurved = sum(Survived)/n())

ggplot(thesummary, aes(x = Pclass, y = Propsurved)) +
  geom_bar(stat="identity") +
  labs(y = "Proportion Who Survived", x = "Class", title = "Survival by Class on the Titanic")
